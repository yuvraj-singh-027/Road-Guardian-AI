...
# Try importing PyMySQL for MySQL
try:
    import pymysql
...
if os.path.exists(os.path.join(BACKEND_DIR, "road_guardian.db")):
    SQLITE_DB_PATH = os.path.join(BACKEND_DIR, "road_guardian.db")
else:
...
def get_db_connection():
    ...
    1. MySQL (MYSQL_HOST, MYSQL_USER, MYSQL_PASSWORD)
    2. Local SQLite fallback (road_guardian.db)
...
def get_db_status() -> Dict[str, Any]:
    ...
    try:
        ...
        target_host = (os.getenv("MYSQL_HOST", "Local Database") if db_type == "mysql" else "Local SQLite File")
        return {
            "type": db_type.upper(),
